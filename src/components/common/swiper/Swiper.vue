...
export default {
  ...
  methods: {
    ...
    setTransform(position) {
      this.swiperStyle.transform = `translate3d(${position}, 0, 0)`;
      // 兼容Safari 和 Chrome
      this.swiperStyle[
        "-webkit-transform"
    ...
    touchStart(e) {
      ...
      this.startX = e.touches[0].pageX;   //鼠标坐标相对于当前页面的坐标
    },

    touchMove(e) {
      this.currentX = e.touches[0].pageX; //鼠标坐标相对于当前页面的坐标
    ...
    touchEnd(e) {
      ...
      if (this.distance === 0) {
        return;
      } else if (this.distance > 0 && (currentMove > this.totalWidth * this.moveRatio)) {//右移超过比例
        this.currentIndex--;
      } else if (this.distance < 0 && (currentMove > this.totalWidth * this.moveRatio)) {//左移超过比例
        this.currentIndex++;
      }
